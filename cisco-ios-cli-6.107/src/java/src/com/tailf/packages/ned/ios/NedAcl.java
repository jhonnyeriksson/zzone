...
public class NedAcl {
    ...
    public String modify(NedWorker worker, String data, int fromTh, int toTh)
        ...
        for (int n = 0; n < lines.length; n++) {
            ...
            if (!m.find()) {
                throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "acl: malformed line: "+line);
            }
            name = m.group(1);
            ...
            if (aclList.isEmpty()) {
                ...
                for (; n < lines.length; n++) {
                    ...
                    if (line.startsWith(" ! move")) {
                        throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "acl: malformed obu: "+line);
                    }
            ...
            for (n = n + 1; n < lines.length; n++) {
                ...
                if (trimmed.startsWith("! ")) {
                    ...
                    if (group == null || Integer.parseInt(group[0]) != 3) {
                        throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "acl: malformed obu: "+nextline);
                    }
                    ...
                    if (group[1].equals("move")) {
                        ...
                        if (index == -1) {
                            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "acl: finding rule to move: '"+rule+"'");
                        }
                    ...
                    if (index == -1) {
                        throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "acl: finding rule for: '"+line+"'");
                    }
                ...
                // no <rule>
                    ...
                    index = listIndexOf(aclList, rule);
                    if (index == -1) {
                        throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "acl: finding '"+rule+"' to delete");
                    }
    ...
    private ArrayList<String[]> listGet(NedWorker worker, String path, int th) throws NedException {
        ...
        try {
            ...
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "acl: listget: "+e.getMessage());
        }
    }
