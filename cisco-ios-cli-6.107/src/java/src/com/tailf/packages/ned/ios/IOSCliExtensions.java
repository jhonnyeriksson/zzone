...
public class IOSCliExtensions implements NedComCliBase.ExtensionsHandler {
    ...
    private static final int DEF_BOOLEAN_SZ = 5;
        ...
        try {
                ...
                if (booleans > 0) {
                    ...
                    for (int i = 0; i < booleans; i++) {
                        ...
                        if (owner.maapi.exists(th, operPath)) {
                            ...
                            owner.maapi.setElem(th, new ConfBuf(mode), operPath+"/mode");
                        }
            ...
            if ("from".equals(dir)) {
                ...
                if (num > 0) {
                    ...
                    List<ConfObject[]> list = owner.maapi.getObjects(cr, DEF_BOOLEAN_SZ, num);
                    for (int n = 0; n < list.size(); n++) {
                        ...
                        traceDev(worker, "booleans: devCmd = "+devCmd);
                        ...
                        traceDev(worker, "booleans: "+shortpath(path)+" CDB="+value+" cmd='"+cmd+"' show-when-set="+showWhenSet);
                        if (devCmd == null && value != null && !isAdd.equals(showWhenSet)) {
                            final String mode = objs[4].toString();
                            if (" :: ".equals(mode)) {
                                traceDev(worker, "booleans: injected "+stringQuote(cmd)+" on show-when-set="+showWhenSet+" device");
                                lines.add(cmd);
                            } else {
                                final String[] modes = mode.split(" :: ");
                                traceDev(worker, "booleans: injected "+stringQuote(modes[0]+cmd+modes[1])+" on show-when-set="+showWhenSet+" device");
                                lines.add(modes[0]);
                                lines.add(cmd);
                                lines.add(modes[1]);
                            }
    ...
    public void configLock(final NedWorker worker, Schema.CallbackMetaData meta,
        ...
        String unlock;
        if (args.length == 1) {
            // kludge: inject hidden dummy entries to detect (e.g. policy-map) chains
            trigger = cmd.replaceFirst(args[0], meta.extArguments).trim() + "\n";
            unlock = "!!"+topModeName(pctx)+"\n";
        } else {
            trigger = cmd.replaceFirst(args[0], args[1]);
            unlock = toEnter(pctx);
            String prefix = "";
            if (meta.extArguments != null) {
    ...
    /**
     * ned-diff-delete-first
     * cli:direction "to-device"
     * cli:state "post-match|call-once"
     */
    public void nedDiffDeleteFirst(final NedWorker worker, Schema.CallbackMetaData meta,
                                   Schema.ParserContext pctx) throws Exception {
        ...
        String inject = toEnter(pctx) + cmd + "\n" + toExit(pctx);
        traceOut(worker, "pre-injected delete: "+stringQuote(inject));
    ...
    public void passwordQuoteInput(NedWorker worker, Schema.CallbackMetaData meta, Schema.ParserContext pctx) {
        ...
        traceDev(worker, "   line-in = '"+line+"'");

        String text = pctx.currentMatch.restOfLine.toString().trim();
        int space = text.indexOf(' ');
        if (space > 0) {
            text = text.substring(0, space);
        }

        final String enc = passwordQuote(text);
        traceIn(worker, "quoted secret to '"+enc+"'");
        pctx.injectImmediate(line.replace(text, enc));
        pctx.skipCurrentLine();
    }
    ...
    private int maapiGetIfAddrs(NedWorker worker, int th, String ifpath, boolean v4only, List<String> inject)
        ...
        try {
            ...
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "internal error in if-vrf-restore: "+e.getMessage(), e);
        }
    ...
    public void ifServiceEvcDeleted(final NedWorker worker, Schema.CallbackMetaData meta,
        ...
        traceOut(worker, "injected service instance to delete EVC "+evcName);
        pctx.skipCurrentLine();
        pctx.injectImmediate(inject);
    }
    ...
    public void ifSwitchPortSpPatch(final NedWorker worker, Schema.CallbackMetaData meta,
        ...
        if (maapiExists(worker, toT, p)) {
            ...
            pctx.injectBefore(" no service-policy xyzoutput "+polname);
        }
        ...
        if (maapiExists(worker, toT, p)) {
            ...
            pctx.injectBefore(" no service-policy xyzinput "+polname);
        }
    }
        ...
        if (!pctx.peekNextLine().trim().startsWith("no key ")) {
            ...
            if (maapiExistsNoException(worker, fromT, path)) {
                ...
                if (num > 0) {
                    ...
                    traceVerbose(worker,
                            "injected mode " + stringQuote(mode) + " after " + stringQuote(pctx.getCurrentLine()));
    ...
    public void booleanDiff(NedWorker worker, Schema.CallbackMetaData meta, Schema.ParserContext pctx)
        ...
        String[] entry = new String[DEF_BOOLEAN_SZ];
        entry[0] = operPath;
        entry[1] = path;
        entry[2] = cmd;
        entry[3] = "?";  // show-when-set
        entry[4] = toEnter(pctx) + " :: " + toExit(pctx);
    ...
    /**
     * Return top-mode CLI command
     * @param
     * @return
     */
    private String topModeName(Schema.ParserContext pctx) {
        List<String> lines = pctx.getEnterContextLines();
        if (lines.isEmpty()) {
            return "";
        } else {
