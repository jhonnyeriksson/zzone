...
public class NedCommand {
    ...
    public String prepare(NedWorker worker, String cmd, ConfXMLParam[] param) throws NedException {
        ...
        if (param != null) {
            ...
            try {
                ...
                throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "nedcommand.prepare() error", e);
            }
        ...
        try {
            ...
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "exec-strict-prompt failed to derive device prompt: "+e.getMessage(), e);
        }
        ...
        if (!xml.trim().isEmpty()) {
            ...
            while (m0.find()) {
                ...
                if (!m.find()) {
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "nedcommand.prepare() failed to extract 'question' in "
        ...
        for (Map<String,String> entry : entries) {
            ...
            if (newEntry[0] == null) {
                throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "missing 'live-status/auto-prompts{"+id+"}/question' ned-setting");
            }
            cmdStrings.add(newEntry);
        }
    ...
    private String doCommand(NedWorker worker, String cmd, boolean single) throws Exception {
        ...
        //
        // Command fastpath - exec-done-pattern-only
        //
        String execDonePatternOnly = owner.nedSettings.getString("live-status/exec-done-pattern-only");
        if (execDonePatternOnly != null) {
            traceInfo(worker, "waiting for live-status/exec-done-pattern-only "+stringQuote(execDonePatternOnly)
                      +" (write-timeout "+owner.writeTimeout+")");
            res = owner.session.expect(new String[] { execDonePatternOnly }, true, owner.writeTimeout, worker);
        ...
        while (true) {
            ...
            if (answer == null) {
                reply.append("\n----- Missing answer to last device question(?) ----- ^\n\n"
            ...
            if ("IGNORE".equals(answer) || "<ignore>".equals(answer)) {
                traceInfo(worker, answer + " -> continue parsing");
