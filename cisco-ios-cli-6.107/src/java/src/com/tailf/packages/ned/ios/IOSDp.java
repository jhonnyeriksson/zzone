...
public class IOSDp {
    ...
    private static org.apache.logging.log4j.Logger log = org.apache.logging.log4j.LogManager.getLogger(IOSDp.class);
    ...
    @DataCallback(callPoint="delete-when-empty-patch",
        ...
        try {
            ...
            // Check NED setting
            final String deviceId = path.replaceFirst(".*/device\\{(\\S+)\\}/config.*", "$1");
    ...
    @DataCallback(callPoint="interface-switchport-hook",
        ...
        try {
            ...
            int th = trans.getTransaction();
            final String path = new ConfPath(keyPath).toString();
            final String deviceId = path.replaceFirst(".*/device\\{(\\S+)\\}/config.*", "$1");
            if (!getNedSettingBoolean(th,deviceId,"cisco-ios/auto/if-switchport-create-hook",false)) {
                return Conf.REPLY_OK;
            }

            // Delete primary and secondary IP address(es)
            final String ifpath = path.replace("switchport", "");
            ...
            final String model = val != null ? val.toString() : "*unknown*";
