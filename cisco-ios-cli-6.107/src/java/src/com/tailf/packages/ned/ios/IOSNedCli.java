...
public class IOSNedCli extends NedComCliBase {
    ...
    private static final String[] staticWarning = {
        ...
        "snmp agent code (disabled|enabled)", // snmp mib nhrp
    ...
    public void nedSettingsDidChange(NedWorker worker, Set<String> changedKeys, boolean isConnected) throws Exception {
        ...
        try {
            ...
            if (showRunningConfig.startsWith("scp-transfer") && proto != null && !"ssh".equals(proto)) {
                throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "ned-settings: must use cli protocol ssh for read/show-running-method scp-transfer");
            }
            ...
            for (Map<String,String> entry : entries) {
                ...
                if (newEntry[2] == null) {
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "ned-settings: "+buf+" missing config");
                }
                if (newEntry[1] != null) {
                    buf += " regexp "+stringQuote(newEntry[1]);
                }
            ...
            for (Map<String,String> entry : entries) {
                ...
                if (newEntry[0] == null || newEntry[1] == null) {
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "ned-settings: read/inject-interface-config "+newEntry[2]
            ...
            for (Map<String,String> entry : entries) {
                ...
                buf += " regexp "+stringQuote(newEntry[1]);
                if (newEntry[1] == null) {
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "ned-settings: write/replace-commit "+newEntry[0]+" missing regexp");
                }
                if (newEntry[2] != null) {
                    buf += " to "+stringQuote(newEntry[2]);
                } else {
            ...
            for (Map<String,String> entry : entries) {
                ...
                newEntry[3] = entry.get("where");
                if (newEntry[3] == null) {
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "ned-settings: write/inject-command "+newEntry[0]+" missing 'where'");
                }
            ...
            for (Map<String,String> entry : entries) {
                ...
                if (newEntry[1] == null || newEntry[2] == null) {
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "ned-settings: write/inject-answer "
    ...
    protected void setupInstance(NedWorker worker, PlatformInfo platformInfo) throws Exception {
        ...
        String[][] defaultAutoPrompts = new String[][] {
            ...
            { " will be deleted:", null }, // install remove [inactive]
            { "\\A\\[[A-Za-z0-9 ]+\\]:", null }, // install remove [inactive]
    ...
    public void show(NedWorker worker, String toptag) throws Exception {
        ...
        try {
            ...
            this.syncFile = null; // clear after input extensions to allow device simulation
        }
        ...
        try {
            ...
            //
                ...
                if (nedSettings.getString("proxy/remote-connection") != null) {
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "ned-settings: read/show-running-method scp-transfer is not supported with proxy mode");
                }
                try {
    ...
    private String showRunningConfig(NedWorker worker) throws Exception {
        ...
        for (int n = 0; n < cmds.length; n++) {
            ...
            if (isExecError(res)) {
                throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "failed to show config using '"+cmds[n]+"'");
            }
            if (cmds.length > 1) {
                res = trimInput(worker, res);
            }
    ...
    private String getConfigOnly(NedWorker worker, String res) throws Exception {
        ...
        try {
            ...
            if (isDevice()) {
                ...
                // IOS changed the hidden default of 'ip bootp server' in IOS version 17.1 or newer
                if (minver != null && Integer.parseInt(minver) >= 1 && !res.contains("\nip bootp server")) {
                    traceInfo(worker, "transformed <= inserted 'no ip bootp server' [IOS 17."+minver+" patch]");
                    first.append("no ip bootp server\n");
                }
            }
    ...
    private void getSnmpUser(NedWorker worker, String dump, int th, StringBuilder sb) throws Exception {
        ...
        try {
            ...
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "getsnmpuser():", ex);
        }
    ...
    private String scpGetConfig(NedWorker worker) throws Exception {
        ...
        for (int retries = retryCount; retries >= 0; retries--) {
            ...
            try {
                ...
                if (retries == 0) {
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "read/show-running-method scp-transfer failed to open scp connection", e);
                } else {
        ...
        if (!scpConn.isAuthenticationComplete()) {
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "read/show-running-method scp-transfer isauthenticationcomplete() = false");
        }
        ...
        try {
            ...
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "scp download exception: "+e.getMessage(), e);
        } finally {
            if (reader != null) {
                reader.close();
            }
        ...
        try {
            ...
            if (isNetsim() && "confd-state-trans-id".equals(readTransIdMethod)) {
                ...
                if (show.contains("error")) {
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "failed to run get confd running transaction-id");
                }
            ...
            // (4) Use 'Last configuration change' string from running-config
                ...
                if (!res.contains("Last configuration change")) {
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "failed to get running-config 'Last configuration change' string");
                }
                res = res + res + res + res;
                which = "LAST-CFG";
            }
            ...
            // (5) Use 'show configuration id' command
                ...
                if (isExecError(res)) {
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "failed to use 'show configuration id' for transaction id");
                }
                res = res + res + res + res;
                which = "CFG-ID";
            }
            ...
            // (6) Use 'show configuration history' command
                ...
                if (isExecError(res)) {
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "failed to use 'show configuration history' for transaction id");
                }
                res = res + res + res + res;
                which = "CFG-HIS";
            }
    ...
    public void applyConfig(NedWorker worker, int cmd, String data)
        ...
        if (failphase.contains("apply")) {
            ...
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "debug :: exception in applyconfig");
        }
    ...
    private void doApplyConfig(NedWorker worker, int cmd, String data)
        ...
        try {
            ...
            try {
                ...
                throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, e.getMessage(), e);
            }
        ...
        try {
            ...
            if (res.contains("Error:")) {
                if (writeIgnoreAbortErrors && cmd == NedCmd.ABORT_CLI) {
                    ...
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, stringQuote(res));
                }
            }
        } catch (Exception e) {
    ...
    private String modifyOutputLine(NedWorker worker, String data, int fromTh, int toTh) throws NedException {
        ...
        for (int n = 0; n < lines.length; n++) {
            ...
            //
                ...
                throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "invalid input, secret type not allowed with algorithm-type in '"+line+"'");
            }
            ...
            //
                ...
                throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "'api ios-common' ned-setting enabled: Use 'logging *' list only");
            }
            ...
            //
                ...
                throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "'"+line+"' : inconsistent address and mask"+modeStack.toString());
            }
            ...
            */
            ...
            //
                ...
                if (line.startsWith("ip access-list ")
                    ...
                    if (maapiExists(worker, toTh, path)) {
                        throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "'"+line+"' : name conflict with an ip access list");
                    }
                }
            }
            ...
            //
                ...
                if (maapiExists(worker, toTh, path)) {
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "'"+line+"' : name conflict with an unordered ip access list");
                }
            }
    ...
    private String modifyNsoOutput(NedWorker worker, String data, int toTh) throws NedException {
        ...
        try {
                        ...
                        for (int l = 0; l < lockList.size(); l++) {
                            ...
                            if (pmConfig != null) {
                                ...
                                if (pmConfig.contains("\n class "+cmName+"\n")) {
                                    traceVerbose(worker, "transformed => injected dummy '"+trigger+"' reference to trigger unlocking mechanism");
                                    sb.append(trigger+"\nexit\n");
                                }
                            }
                        }
            ...
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "internal error in modifyNsoOutput: " + e.getMessage(), e);
        }
    }
    ...
    private String reorderOutput(NedWorker worker, String data) throws NedException {
        ...
        for (int n = 0; n < lines.length; n++) {
            ...
            // interface * / no service-policy input|output - extract and inject first
            else if (line.startsWith(" no service-policy xyz")) {
                middle.append(line.replace(" xyz"," ")+"\n"); //  ios:if-switchport-sp-patch kludge to avoid below reordering
            }

            // interface * / no service-policy input|output - extract and inject first
            else if (toptag.startsWith("interface ")
                     && (line.startsWith(" no service-policy output ") || line.startsWith(" no service-policy input "))) {
                traceVerbose(worker, "transformed => injected interface '"+line.substring(4)+"' delete first");
                final String inject = toptag+"\n"+line+"\n"+"exit\n";
                if (toptag.contains(".")) {
                    first.insert(0, inject); // inject delete on sub-interface before others
                } else {
                    first.append(inject);
                }
            }
    ...
    protected void enterConfig(NedWorker worker, String data) throws NedException, IOException, SSHSessionException {
        ...
        for (int n = 1; n < 10; n++) {
            ...
            default:
                ...
                throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "config terminal: "+expectGetMatch(res));
            }
        }
        ...
        if (res != null) {
            ...
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "config terminal: timed out");
        }
    }
        ...
        try {
            ...
            while (retry && minutes > 0) {
                ...
                default:
                    ...
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, lastText+expectGetMatch(res));
                }
            }
    ...
    private void print_line_wait(NedWorker worker, int cmd, String line, int retrying, String toptag, int num)
        ...
        for (int loops = 0; loops < 3; loops++) {
            ...
            if (res.getHit() >= PLW0.length) {
                ...
                traceInfo(worker, "matched write/inject-answer "+entry[0]+": injected answer "+stringQuote(entry[2]));
    ...
    public boolean isNetsim() {
        return (iosmodel.contains("NETSIM") && syncFile == null);
    }
