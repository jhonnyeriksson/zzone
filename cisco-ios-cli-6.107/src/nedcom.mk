...
YANG_CONFIG ?= $(sort $(filter-out tailf-%-dev-meta.yang tailf-%-meta-custom.yang $(YANG_STATS) $(YANG_OTHER),$(YANG)))
...
NED_ID_TOOL_ARG := $(strip $(if $(NED_ID_SUFFIX),-s $(NED_ID_SUFFIX)) $(if $(NED_ID_MAJOR),-M $(NED_ID_MAJOR)) $(if $(NED_ID_MINOR),-m $(NED_ID_MINOR)) -N $(NCS_VERSION))
...
link_extra_deviation:
	(cd tmp-yang/config; for f in $(YANG_EXTRA_DEVIATION_FILE); do ln -s $$f .; done)
...
link_scoped_config_yang:
	...
	@(cd tmp-yang/config; ln ../tailf-internal-rpcs.yang .; ln ../tailf-internal-rpcs-custom.yang .)
