...
def version(nso_version):
    m = re.search(r'(\d+)\.(\d+).*', nso_version)
    return int(m.group(1))*10 + int(m.group(2))

def generate_files(name, ned_id_suffix, major, minor, nso_version, print_only):
    ...
    with open(name,'r') as template:
        ...
        if not print_only:
            ...
            xml = re.sub(r'(<name>)[^<]+(</name>)', '\g<1>%s\g<2>' % ned_id, xml, 1)
            xml = re.sub(r'(<ned-id)[^>]+>[^<]+(</ned-id>)',
                         '\g<1> xmlns:id=\"http://tail-f.com/ns/ned-id/%s\">id:%s\g<2>' % (ned_id,ned_id), xml)
            xml = xml.replace('<?xml version="1.0"?>\n', '')

            if version(nso_version) < version("6.3"):
                # Trim new device info tags from xml dump since they are not
                # compatible with pre 6.3 NSO
                xml = re.sub(r'[\s\n]*<product-family>[^<]+</product-family>','', xml)
                xml = re.sub(r'[\s\n]*<operating-system>[^<]+</operating-system>','', xml)
                xml = re.sub(r'[\s\n]*<management-protocol>[^<]+</management-protocol>', '', xml)
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv,'f:s:m:M:pN:',['help','file=','suffix=','minor=','major=','print-only','nso-version='])
    ...
    for opt, arg in opts:
        if opt in ('-h','--help'):
            ...
            minor = arg
        elif opt in ('-N', '--nso-version'):
            nso_version = re.search(r'([0-9\.]+)', arg).group(1)
    ...
    ned_id = generate_files(template, suffix, major, minor, nso_version, print_only)
