...
public class IosxrNedCli extends NedComCliBase {
    ...
    private String lastTraceFileName;
    ...
    public void nedSettingsDidChange(NedWorker worker, Set<String> changedKeys, boolean isConnected) throws Exception {
        ...
        try {
            ...
            if ("robust-mode".equals(parser) || "disabled".equals(parser)) {
                throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "ned-settings: extended-parser "+parser+" is deprecated, please change value");
            }
            ...
            for (Map<String,String> entry : entries) {
                ...
                if (newEntry[1] == null) {
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "ned-settings: read/replace-config "+newEntry[0]+" missing regexp");
                }
                if (newEntry[2] != null) {
            ...
            for (Map<String,String> entry : entries) {
                ...
                if (newEntry[1] == null || newEntry[2] == null || newEntry[3] == null) {
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "write/inject-command "+newEntry[0]
            ...
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "failed to read ned-settings :: "+e.getMessage(), e);
        }
    ...
    protected PlatformInfo setupDevice(NedWorker worker) throws Exception {
        ...
        try {
            ...
            if (version.contains("Cisco IOS XR Admin Software")) {
                ...
                throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "unknown device :: " + version);
            }
            ...
            //
                ...
                if (platformModelRegex != null) {
                    ...
                    if (m.find()) {
                        ...
                        throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "no match for platform-model-regex = "+stringQuote(platformModelRegex));
                    }
                } else if ((match = getMatch(version, "\ncisco (8[1-9]\\S+) ")) != null) {
                    this.iosmodel = match;  // ignore generic 'cisco 8000' lines first
                } else {
                    p = Pattern.compile("\ncisco (.+?) (?:Series |\\().*");
                    m = p.matcher(version);
                    if (m.find()) {
            ...
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "failed to setup ned :: "+e.getMessage(), e);
        } finally {
            if (th != -1) {
    ...
    public void show(NedWorker worker, String toptag) throws Exception {
        ...
        if (session != null && nedSettings.getBoolean("read/transaction-id-provisional")) {
        ...
        try {
            ...
            // devices device <dev> live-status exec any sync-from-file <file>
                ...
                if (res.contains("Error: failed to open file")) {
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "failed to sync from file " + syncFile);
                }
            }
            ...
            // sftp-transfer
                ...
                if (remoteConnection != null) {
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "sftp-transfer is not supported with proxy mode");
                }
                if (readDeviceFile == null) {
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "no sftp file name configured");
                }
            ...
            if (isDevice() && session != null) {
                ...
                // (optional) Prepend all interface config
                if (showHiddenInterfaces) {
    ...
    private String getAdminConfig(NedWorker worker, boolean enterAdmin)
        ...
        if (inConfig) {
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "getadminconfig() :: internal error: called in config mode");
        }
        ...
        try {
            ...
            if (isExecError(res)) {
                throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "internal error: failed to run 'show running-config' in admin mode");
            }
        } finally {
            if (enterAdmin) {
                exitAdmin(worker);
    ...
    private NedSSHClient sftpConnect(NedWorker worker) throws NedException {
        ...
        try {
            ...
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "sftp failed (check device ssh config or disable sftp in ned-settings) :: "
                                   + e.getMessage());
        }
    }
    ...
    private String sftpGetConfig(NedWorker worker) throws Exception {
        ...
        if (reply.startsWith(CMD_ERROR)) {
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "sftp-transfer error: "+reply.replace(CMD_ERROR, ""));
        }
        if (reply.contains("No such file or directory")) {
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "sftp-transfer error copying running-config to file, check 'read file' ned-setting");
        }
        ...
        //
            ...
            try {
                for (String cmdPath : pathsToDump) {
                    ...
                    if (dump == null) {
                        throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "showpartial() :: failed to show '"+cmdPath+"'");
                    }
                    if (!dump.isEmpty()) {
    ...
    private String getCommitId(NedWorker worker) throws NedException, IOException, SSHSessionException {
        ...
        if (res.contains("The commit database is empty")) {
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "empty commit database");
        }
        ...
        if (!isExecError(res)) {
            ...
            if (i > 0) {
                ...
                throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "failed to find commit list id: "+stringQuote(res));
            }
        }
        ...
        // Failure
        throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "failed to show last commit list id");
    }
    ...
    public void prepareDry(NedWorker worker, String data) throws Exception {
        ...
        try {
            ...
            if (delayedCommit.length() > 0) {
                ...
                sb.append("commit\n");
                sb.append(modifyDelayedOutput(delayedCommit.toString()));
            }
    ...
    private void doApplyConfig(NedWorker worker, String data)
        ...
        try {
            ...
            if (!data.trim().isEmpty()) {
                if (adminLogin) {
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "admin mode login, can't commit non-admin config");
                }
                sendConfig(worker, data, true, writeChunkSize);
                // In config mode
            }

            // Trigger NedSecrets extensions on delayedCommit config
            if (delayedCommit.length() > 0) {
                traceInfo(worker, "handling delayedcommit cli:secret extensions");
                parseCLIDiffWithExtensions(worker, delayedCommit.toString(), true,
                                           Arrays.asList(new String[]{"cli:secret"}));
    ...
    private String commitAdminConfig(NedWorker worker, String data)
        ...
        if (!readAdminShowRun) {
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "commiting admin config not supported with ned-setting "
        ...
        if (!enterAdmin(worker)) {
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "admin mode has been deprecated, disable it with ned-settings");
        }
    ...
    private void doSendConfig(NedWorker worker, String[] lines, int chunkSize)
        ...
        try {
            ...
            if ("prepare".equals(failphase)) {
                failphase = "";
                throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "prepare :: debug exception in prepare");
            }
            ...
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, e.getMessage()+modeStack.toString(), e);
        }
    }
    ...
    private String maapiDecryptLine(NedWorker worker, String line) {
        ...
        while (m.find()) {
            ...
            try {
                ...
                String clear = mCrypto.decrypt(password);
                if (isNetsim() && !clear.startsWith("\"") && (clear.contains(";") || clear.contains("!"))) {
                    clear = "\"" + clear + "\"";
                }
                traceVerbose(worker, "transformed => maapi.decrypted '"+password+"'");
                line = line.substring(0, m.start()+1) + clear + line.substring(m.end(), line.length());
            } catch (Exception e) {
    ...
    /**
     * Prepare final output data, e.g. trim route-policy ""
     * @param
     * @return
     */
    private String modifyDelayedOutput(String data) {
        String[] lines = data.split("\n");
        StringBuilder sb = new StringBuilder();
        for (String line : lines) {
            if (line.trim().startsWith("route-policy ")) {
                line = line.replace("\"", ""); // Strip quotes around name
            }
            sb.append(line+"\n");
        }
        return sb.toString();
    ...
    private String editOutputRoutePolicyData(NedWorker worker, String data, int toTh) throws NedException {
        ...
        try {
            ...
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "editoutputroutepolicydata '"+lines[n]+"' error: "+e.getMessage(), e);
        }
        return data;
    }
    ...
    private String modifyOutputSets(NedWorker worker, String data, int toTh) throws NedException {
        ...
        try {
            ...
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "modifyOutputsets '"+lines[n]+"' error: "+e.getMessage(), e);
        }
        return data;
    }
    ...
    private boolean isCliError(NedWorker worker, String reply, String line) {
        ...
        // Kludge to ignore missing 'evpn / evi * / control-word-disable' delete when srv6
        if ("no control-word-disable".equals(line.trim())
            && reply.contains("Invalid input detected at")) {
            traceInfo(worker, "ignoring delete of non-existing config "+stringQuote(line));
            return false;
        }
        ...
        for (int retry = writeOobExclusiveRetries; retry >= 0; retry--) {
            ...
            // "Do you wish to proceed with this commit anyway?"
                ...
                if ("no".equals(commitOverrideChanges)) {
                    ...
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, res.getText()+msg0+msg1);
                }
            ...
            if (!line.trim().startsWith("commit")) {
                ...
                throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, reply);
            }
        ...
        while (true) {
            ...
            case 7:
                ...
                if (connAdminName == null) {
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "failed to enter admin mode, missing 'connection admin name' ned-setting");
                }
            ...
            case 8:
                ...
                if (password == null) {
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "failed to enter admin mode, "
            ...
            case 9:
                ...
                break;
            case 10:
                if (iosversion.startsWith("7") || iosversion.startsWith("24")) {
                ...
                throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "failed to enter admin mode: "+stringQuote(res.getText())
                                       +"\nSet ned-setting read/admin-show-running-config to false");
            case 11:
                traceInfo(worker, "entered admin mode");
                return true;
            default:
                throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "failed to enter admin mode: "+res.getText());
            }
        }
    }
    ...
    protected void enterConfig(NedWorker worker) throws IOException, SSHSessionException, NedException {
        ...
        for (int retries = writeOobExclusiveRetries; retries >= 0; retries--) {
            ...
            if (res.getHit() == 0) {
                ...
                throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, line+": "+res.getText());
            }
            sleep(worker, 1000, true);
        }
    ...
    protected void exitConfig(NedWorker worker, String reason) throws IOException, SSHSessionException, NedException {
        ...
        if (!inConfig) {
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "internal error, tried to exit non-config mode");
        }
        ...
        case 2:  // Invalid input detected at
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "failed to exit config mode: "+res.getText());
        default: // exec mode
            break;
        }
        ...
        try {
            ...
            for (int i = 0; i < 30; i++) {
                ...
                case 1: // Invalid input detected at
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, res.getText());
                case 2: // config sub-mode
                    break;
                default:
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "in exec mode");
                }
            }

            // (4) Failed
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "exit not accepted?");

        } catch (Exception e) {
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "failed to move to top mode: "+e.getMessage(), e);
        }
    }
    ...
    protected String editListToStringBuilder(NedWorker worker, String path, int th, StringBuilder sb, boolean ext)
        ...
        try {
            ...
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "edit: editlisttostringbuilder error : "+e.getMessage(), e);
        }
    }
    ...
    private int setListToStringBuilder(NedWorker worker, String path, int th, StringBuilder sb)
        ...
        try {
            ...
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "set: setlisttostringbuilder error : "+e.getMessage(), e);
        }
    }
    ...
    private void sftpPutConfig(NedWorker worker, String data)
        ...
        if (remoteConnection != null) {
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "sftp apply error: Using sftp to apply config is not supported via proxy");
        }
        if (writeDeviceFile == null || writeDeviceFile.isEmpty()) {
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "sftp apply error: no file name configured");
        }
        ...
        try {
            ...
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "sftp apply error : " + e.getMessage());
        } finally {
            if (client != null) {
                client.close();
            }
        ...
        if (res.contains("Couldn't open file")) {
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "sftp load: "+res);
        }
        if (res.contains("Syntax/Authorization errors in one or more commands")) {
            ...
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "sftp apply: "+res);
        }
    ...
    private void doCommit(NedWorker worker, boolean allowTrial) throws Exception {
        ...
        try {
            ...
            if ("before commit".equals(failphase)) {
                failphase = "";
                throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "commit :: simulated exception before commit");
            }
            ...
            if ("after commit".equals(failphase)) {
                failphase = "";
                throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "commit :: simulated exception after commit");
            }

            // Send and commit delayedCommit
            if (delayedCommit.length() > 0) {

                String data = modifyDelayedOutput(delayedCommit.toString());
                ...
                if ("before delayed".equals(failphase)) {
                    failphase = "";
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "commit :: simulated exception before delayed commit");
                }
                ...
                if ("after delayed".equals(failphase)) {
                    failphase = "";
                    throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "commit :: simulated exception after delayed commit");
                }
            }
    ...
    private void internalCmdCheckConfigDir(NedWorker worker, String cmd, StringBuilder reply, boolean unique)
        ...
        if (files == null || files.length == 0) {
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "no such directory or no trace file in: "+dir);
        }
        ...
        if (totalTraces == 1) {
            reply.append("\nparsed "+lastTraceFileName+" containing "+totalLines
                         +" lines with "+unknownLines+" unknown lines (parse-time "+totalTime+")");
        } else {
    ...
    protected boolean exitPrompting(NedWorker worker) throws IOException, SSHSessionException, NedException {
        ...
        throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "exitprompting :: internal error: failed to exit, report with raw trace");
    }
    ...
    protected void reconnectDevice(NedWorker worker) throws NedException {
        ...
        try {
            ...
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "failed to reconnect :: "+e.getMessage(), e);
        }
    }
    ...
    private void maapiAttach(NedWorker worker, int fromTh, int toTh) throws NedException {
        try {
            ...
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "internal error: maapiattach()"+e.getMessage(), e);
        }
    }
    ...
    protected boolean maapiExists(NedWorker worker, int th, String path) throws NedException {
        ...
        try {
            if (maapi.exists(th, path)) {
                traceDev(worker, "maapiExists("+shortpath(path)+") = true");
                return true;
            }
        } catch (Exception e) {
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "maapiexists("+path+") error : " + e.getMessage());
        }
        traceDev(worker, "maapiExists("+shortpath(path)+") = false");
        return false;
    }
    ...
    protected String maapiGetConfig(NedWorker worker, int th, String path, int trimLevel) {
        ...
        try {
            ...
            MaapiInputStream in = maapi.saveConfig(th,
                                                   ...
                                                   path);
            if (in == null) {
                traceInfo(worker, "maapiGetConfig("+shortpath(path)+") error: null config");
                return null;
            }
            ...
            traceInfo(worker, "maapiGetConfig("+shortpath(path)+") error: Exception "+ e.getMessage());
            return null;
        }

        String[] lines = sb.toString().split("\n");
        if (lines.length < 5) {
            traceVerbose(worker, "maapiGetConfig("+shortpath(path)+") length = "+lines.length);
        ...
        traceDev(worker, "maapiGetConfig("+shortpath(path)+","+trimLevel+") =\n"+data);
        return data;
    }
    ...
    protected ArrayList<String[]> maapiGetObjects(NedWorker worker, int th, String path, int numLeaves)
        ...
        try {
            ...
            throw new NedException(NedErrorCode.NED_EXTERNAL_ERROR, "internal error in maapigetobjects(): " + e.getMessage(), e);
        }
    }
    ...
    protected String maapiGetLeafString(NedWorker worker, int th, String path) {
        ...
        try {
            if (maapi.exists(th, path)) {
                String value = ConfValue.getStringByValue(path, maapi.getElem(th, path));
                traceDev(worker, "maapiGetLeafString("+shortpath(path)+") = "+value);
                return value;
            }
        } catch (Exception e) {
     ...
     */
    protected String printLineExec(NedWorker worker, String line) throws IOException, SSHSessionException {
     ...
     * @throws IOException, SSHSessionException
     */
    protected String printLineExec(NedWorker worker, String line, int timeout)
        throws IOException, SSHSessionException {
